        ...
        wm.getWeather(function(weather) {
            ...
            dbModule.getUsers(pageNo, function(users) {
                dbModule.getUserCount(function(result) {        // 페이지 지원
                    ...
                    let view = require('./view/listItem');
    ...
    if (req.session.userId === undefined) {
        ...
        wm.getWeather(function(weather) {
            ...
            dbModule.getAllDepts(function(rows) {
                let view = require('./view/registerItem');
...
router.post('/register', function(req, res) {
    ...
    dbModule.getUserInfo(uid, function(row) {
        ...
        if (row[0] === undefined) {
            if (pswd.length < 4) {
                let html = alert.alertMsg('패스워드 길이가 너무 작습니다.', '/item/register');
                res.send(html);
            } else if (pswd === pswd2) {
                ...
                let html = alert.alertMsg('패스워드가 일치하지 않습니다.', '/item/register');
                res.send(html);
            }
        } else {
            let html = alert.alertMsg(`${uid} 아이디가 중복입니다.`, '/item/register');
            res.send(html);
        }
    });
});
        ...
        wm.getWeather(function(weather) {
            ...
            dbModule.getAllDepts(function(depts) {
                dbModule.getUserInfo(uid, function(user) {
                    //console.log(user[0]);
                    let view = require('./view/updateItem');
    ...
    dbModule.getUserInfo(uid, function(user) {
        if (changePswd === undefined) {         // 패스워드 변경 체크박스가 uncheck 되었을 때
            ...
            if (oldPswd !== user.password) {    // 현재 패스워드가 틀렸을 때
                let html = alert.alertMsg(`현재 패스워드가 틀립니다.`, `/item/update/uid/${uid}`);
                res.send(html);
            } else if (pswd.length < 4) {        // 입력한 패스워드의 길이가 4 미만일 때
                let html = alert.alertMsg(`신규 입력한 패스워드의 길이가 작습니다.`, `/item/update/uid/${uid}`);
                res.send(html);
            } else if (pswd !== pswd2) {        // 입력한 패스워드가 다를 때
                let html = alert.alertMsg(`신규 입력한 패스워드가 다릅니다.`, `/item/update/uid/${uid}`);
                res.send(html);
            } else {            // 모든 조건을 만족시켰을 때
...
router.get('/delete/uid/:uid', function(req, res) {     // 관리자로 로그인해야 할 수 있음.
    if (req.session.userId === undefined) {
        ...
        let html = alert.alertMsg(`사용자를 삭제할 권한이 없습니다.`, '/item/list/page/1');
        res.send(html);
    } else {
        let uid = req.params.uid;
        if (uid === 'admin') {
